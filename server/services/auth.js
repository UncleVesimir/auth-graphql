...
const INVALID_CREDENTIALS_MESSAGE = "Oops! Those credentials are invalid"
...
passport.use(new LocalStrategy({ usernameField: 'email' }, (email, password, done) => {
  User.findOne({ email: email.toLowerCase() }, (err, user) => {
    ...
    if (!user) { return done(null, false, INVALID_CREDENTIALS_MESSAGE); }
    user.comparePassword(password, (err, isMatch) => {
      if (err) { return done(err); }
      if (isMatch) {
        return done(null, user);
      }
      return done(null, false, INVALID_CREDENTIALS_MESSAGE);
    });
  });
}));
...
function login({ email, password, req }) {
  return new Promise((resolve, reject) => {
    passport.authenticate('local', (err, user) => {
      if (!user) { reject(INVALID_CREDENTIALS_MESSAGE) }
